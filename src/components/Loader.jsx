...
export default function Loader({ onDone, gateReady = true }) {
  ...
  useEffect(() => {
    ...
    const totalDraw = SEG1_DUR + SEG2_DUR
    ...
    tl.to(wrapper, { opacity: 1, duration: 0.4, ease: 'power2.out' })
    tl.to(
      path,
      { strokeDashoffset: length * 0.55, duration: SEG1_DUR, ease: 'power2.inOut' },
      '<',
    )
    tl.to(path, { strokeDashoffset: 0, duration: SEG2_DUR, ease: 'power1.out' })
    tl.to(
      counter,
      {
        value: 100,
