...
const TEXT =
  "I'm Yahaya. I design products that try to feel like they were made by a person who cared. Most software lands cold, so I try to leave warmth in the seams. A turn of phrase. A moment of friction earned. A small joy hidden in the corner of a screen. The work I love best treats people like the smart, busy humans they are. It moves out of their way when it should, and rises to meet them when it matters. This is how I work, and what I am building towards next."

const START_DELAY = 1200
const MIN_KEY = 28
const MAX_KEY = 105
const PAUSE_AFTER_PERIOD = 380
const PAUSE_AFTER_COMMA = 180
const PAUSE_AFTER_COLON = 240
const SIGNATURE_DELAY = 700

function nextDelay(prevChar) {
  if (prevChar === '.' || prevChar === '!' || prevChar === '?') {
    return PAUSE_AFTER_PERIOD + Math.random() * 120
  }
  if (prevChar === ',') return PAUSE_AFTER_COMMA + Math.random() * 80
  if (prevChar === ':' || prevChar === ';') {
    return PAUSE_AFTER_COLON + Math.random() * 80
  }
  return MIN_KEY + Math.random() * (MAX_KEY - MIN_KEY)
}

function Signature({ play, reduced }) {
  ...
  useEffect(() => {
    ...
    // Two segments simulate brief pressure variation in a real signature.
    tl.to(
      path,
      {
  ...
  return (
    <div ref={wrapperRef} className="manifesto__sig">
      <svg
        width="244"
        height="88"
        viewBox="0 0 122 44"
...
export default function Manifesto() {
  ...
  // runId increments each time the section re-enters view, restarting typing.
  const [runId, setRunId] = useState(0)
  const [active, setActive] = useState(false)
  const [typed, setTyped] = useState('')
  const [done, setDone] = useState(false)

  // Pause / restart on viewport entry. Threshold is intentionally low so that
  // the auto-scroll-as-you-type behaviour doesn't accidentally trip a restart.
  useEffect(() => {
  ...
  // Keep the currently-writing line in the viewport. Only scrolls down when
  // the paragraph's bottom drifts below the target band — i.e. when text
  // wraps onto a new line. Uses Lenis if available for smooth scrolling.
  const followTyping = () => {
    const el = textRef.current
    if (!el) return
    const rect = el.getBoundingClientRect()
    // Skip if the paragraph didn't grow taller (we're still on the same line).
    if (Math.abs(rect.bottom - lastBottomRef.current) < 2) return
    lastBottomRef.current = rect.bottom

    const target = window.innerHeight * 0.55
    if (rect.bottom <= target) return

    const delta = rect.bottom - target
    const lenis = getLenis()
    const nextTop = window.scrollY + delta
    if (lenis) {
      lenis.scrollTo(nextTop, { duration: 0.6 })
    } else {
      window.scrollTo({ top: nextTop, behavior: 'smooth' })
    }
  }

  // Type out the manifesto. Re-runs whenever runId changes (new entry).
  useEffect(() => {
    if (!active) return
    ...
    const tick = () => {
      ...
      // After the DOM updates, nudge the page so the writing line stays in view.
      requestAnimationFrame(followTyping)
      timer = setTimeout(tick, nextDelay(justTyped))
    }
  ...
  }, [active, runId, reduced, play])

  const showSignature = done
  const showCursor = !done

  return (
    <section ref={containerRef} className="manifesto">
      <p ref={textRef} className="manifesto__text" aria-label={TEXT}>
        <span className="manifesto__typed" aria-hidden="true">
          {typed}
        </span>
        {showCursor && (
          <span
            className={`manifesto__caret ${typed.length === 0 ? 'manifesto__caret--idle' : ''}`}
            aria-hidden="true"
          />
        )}
      </p>
      ...
      <div
        className="manifesto__sig-wrap"
        style={{
          transitionDelay: showSignature ? `${SIGNATURE_DELAY}ms` : '0ms',
        }}
      >
        {showSignature && (
          <>
